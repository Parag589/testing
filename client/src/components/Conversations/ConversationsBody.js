...
const ConversationBody = ({ chatmessages, selectedSenderName,user1, user2, uniqueId1, uniqueId2, handleChatClick}) => {
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post('http://localhost:8000/send-message', {
        senderId: user2,
        recipientId: uniqueId2, // Replace with the actual user ID
        message: {
          text: sendmessage,
        },
