  ...
  return (
    <div
      className={` w-fit py-2 ${
        message?.from.id === pageData.id ? " ml-auto mr-6" : "ml-6 "
      }`}
    >
      <div
        className={`flex gap-2 ${
          message?.from.id === pageData.id ? " flex-row-reverse " : ""
        }`}
      >
        <div className="relative top-2">
          <img
            src={message?.from.id === pageData.id ? receiverPic : senderPic}
            alt="user-icon"
